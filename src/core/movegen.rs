...
use crate::models::chessmove::{ChessMove, Square};
use crate::models::piece::Piece;

pub fn possible_moves(board: &ChessBoard) -> Vec<ChessMove> {
    let all_boards = board.get_bitboards();
    let player_boards: [Bitboard; 6] = if board.get_white_to_move() {
        all_boards[0..6].try_into().unwrap()
    } else {
        all_boards[6..12].try_into().unwrap()
    };
    let index_offset = if board.get_white_to_move() { 0 } else { 6 };
    ...
    let all_pieces: Bitboard = board.get_all_pieces();
